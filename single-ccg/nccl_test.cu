...
__global__ void initSendBuffer(float* sendbuff, int size, int rank) {
  for (int i = 0; i < size; i++) {
    sendbuff[i] = (float)rank;
  }
}
...
int main(int argc, char* argv[])
{
  ...
  initSendBuffer<<<1,1>>>(sendbuff, size, myRank);
  ...
  printf("[MPI Rank %d] recvbuff values:\n", myRank);
  for (int i = 0; i < 10; i++) {
